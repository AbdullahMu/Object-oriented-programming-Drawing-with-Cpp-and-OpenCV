...
void RGBMatrix::draw(Shape shape) {
    ...
    auto fill_color = color->getRGBColor();

    std::vector<Point> fill_points = shape.getPoints();

    for (Point point : fill_points) {
        if (point.x <= m_rows && point.y <= m_cols) {
            for (size_t channel = 0; channel < 3; channel++)
                at(point.x, point.y, channel) = fill_color[channel];
        }
    }
